...
export function buildStatPacket(input: PacketInput): StatPacket {
  ...
  const teams: SeasonTeamLine[] = teamWeeks.map((team) => {
    ...
    return {
      ...
      benchRegret: report.regrets.slice(0, 3).map((regret) => ({
        player: regret.benchPlayer.name,
        playerId: regret.benchPlayer.id,
        points: regret.benchPoints,
        replacing: regret.replacing?.name ?? null,
        replacingPoints: regret.replacingPoints,
        delta: regret.delta,
        slot: regret.slot,
      })),
